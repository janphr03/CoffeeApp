...
//FRONTEND_URL aus .env, sonst Default
...
async function checkStart(page) {
    const resp = await page.goto(FRONTEND_URL, { timeout: TIMEOUT, waitUntil: 'domcontentloaded' });
...
async function checkMap(page) {
    const resp = await page.goto(`${FRONTEND_URL}/map`, { timeout: TIMEOUT, waitUntil: 'domcontentloaded' });
...
async function run() {
    ...
    for (const b of browsers) {
        console.log(`\n[${b.name}] Starte Tests gegen ${FRONTEND_URL}...`);
